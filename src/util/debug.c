...
void debug_ecs(const ECS* ecs) {
	...
	camera_begin_drawing(ecs->camera);
		...
		// Collision
		const Rectangle* rect_begin = (Rectangle*) vector_begin(ecs->static_collisions);
		const Rectangle* rect_end = (Rectangle*)vector_end(ecs->static_collisions);
		for (const Rectangle* rect = rect_begin; rect < rect_end; rect++) {
		...
		while ((e = (entity_t*)set_iter_next(players)) != NULL) {
			const Player* player = ecs_get_component(ecs, *e, PLAYER_ID);
